...
struct node head_app;
struct node head_lib;

char *APP_NAME = "";
char *LIB_NAME = "./fake_lib.so";
...
static int callback(struct dl_phdr_info *info, size_t size, void *data)
{
	...
	if (!strcmp(APP_NAME, info->dlpi_name)) {
	    app_index = num_load_addresses;
	} else if (!strcmp(LIB_NAME, info->dlpi_name)) {
        lib_index = num_load_addresses;
	}
        num_load_addresses++;
    }
    return 0;
...
void read_in_file(void) {
    ...
    head_app.next = &head_app;
    head_app.prev = &head_app;

    head_lib.next = &head_lib;
    head_lib.prev = &head_lib;

    FILE *fp = fopen("/tmp/vprof/var_info_app", "r");
    if (fp) {
        read_in_file_helper(&head_app, fp);
    }

    fp = fopen("/tmp/vprof/var_info_lib", "r");
    if (fp) {
        read_in_file_helper(&head_lib, fp);
    }
}
